...
class Particle {
	...
	constructor(w: number, h: number) {
		...
		if (roll < 0.4) {
			...
			if (colorRoll < 0.5)
				this.color = COLORS.lightBlue.replace("OPACITY", "1");
		...
		if (!reducedMotion) {
			...
			if (mouseActive && mouseX > 0 && mouseSpeedMultiplier > 0.01) {
				...
				if (dist < radius && dist > 0) {
					const strength =
						(1 - dist / radius) ** 2 * this.z * mouseSpeedMultiplier;
...
export function AntigravityParticleField() {
	...
	useEffect(() => {
		...
		const render = (time: number) => {
			...
			const timeSinceMove =
				performance.now() - globalMouse.current.lastMoveTime;
			const isMoving = timeSinceMove < 150;
			const mouseSpeedMultiplier = isMoving
				? 1
				: Math.max(0, 1 - (timeSinceMove - 150) / 400);
