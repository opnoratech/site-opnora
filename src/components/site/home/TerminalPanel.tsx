...
const LINES: Line[] = [
  ...
  { type: "out", text: "" },
  { type: "status", label: "Opnora Build", value: "ATIVO", color: "text-[#4ed4cf]" },
  { type: "status", label: "Opnora Intelligence", value: "ATIVO", color: "text-[#58e5a6]" },
  { type: "status", label: "Opnora Labs", value: "EM EVOLUÇÃO", color: "text-[#c084fc]" },
  { type: "out", text: "" },
  { type: "prompt", text: "~/opnora $", cmd: "get mission --brief" },
  { type: "out", text: "Desenvolver software e transformar desafios em soluções reais." },
  { type: "out", text: "" },
];
  ...
  useEffect(() => {
    ...
    const runAnimation = async () => {
      ...
      for (let i = 0; i < LINES.length; i++) {
        ...
        else if (line.type === "out" && line.text.includes("inicializando")) delay = 400;
            ...
            if (line.type === "status") {
              return (
                ...
                >
                  ...
                  <span className={`${line.color} font-bold tracking-wide`}>[{line.value}]</span>
                </div>
              );
            }
          })}
