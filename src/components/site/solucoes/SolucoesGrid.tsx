...
import { FaCheck, FaCode, FaFlaskVial, FaMicrochip } from "react-icons/fa6";
              ...
              <div className="max-w-[85rem] mx-auto px-6 lg:px-12 relative z-10">
                ...
                >
                  ...
                  <div className="relative w-full lg:w-1/2 flex items-center justify-center min-h-[280px] sm:min-h-[360px] md:min-h-[420px] lg:min-h-[480px] z-10">
                    <ScrollReveal
                      delay={100}
                      className="relative w-full h-full flex items-center justify-center group py-8 cursor-pointer md:cursor-default outline-none"
                      onClick={() =>
                        setActiveSection(activeSection === section.num ? null : section.num)
                      }
                      data-active={activeSection === section.num}
                      style={{ WebkitTapHighlightColor: "transparent" }}
                    >
                      {/* Big Number Background */}
                      <div className="absolute inset-0 flex items-center justify-center pointer-events-none z-0 overflow-visible">
                        <span
                          className="font-display text-[10rem] sm:text-[14rem] md:text-[18rem] lg:text-[22rem] font-bold leading-none select-none transition-transform duration-700 ease-out group-hover:scale-105 group-data-[active=true]:scale-105"
                          style={{
                            color: section.color,
                            opacity: 0.07,
                      ...
                      <div className="absolute inset-0 flex items-center justify-center pointer-events-none opacity-50 z-1">
                        <div
                          className="absolute w-24 h-24 sm:w-32 sm:h-32 md:w-40 md:h-40 rounded-full border group-hover:scale-110 group-data-[active=true]:scale-110 transition-transform duration-700 ease-out"
                          style={{ borderColor: `rgba(${section.rgb}, 0.45)` }}
                        />
                        <div
                          className="absolute w-36 h-36 sm:w-48 sm:h-48 md:w-60 md:h-60 rounded-full border group-hover:scale-110 group-data-[active=true]:scale-110 transition-transform duration-700 ease-out delay-75"
                          style={{ borderColor: `rgba(${section.rgb}, 0.28)` }}
                        />
                        <div
                          className="absolute w-52 h-52 sm:w-68 sm:h-68 md:w-84 md:h-84 rounded-full border group-hover:scale-110 group-data-[active=true]:scale-110 transition-transform duration-700 ease-out delay-150"
                          style={{ borderColor: `rgba(${section.rgb}, 0.18)` }}
                        />
                        <div
                          className="absolute w-72 h-72 sm:w-96 sm:h-96 md:w-[28rem] md:h-[28rem] rounded-full border group-hover:scale-110 group-data-[active=true]:scale-110 transition-transform duration-700 ease-out delay-200"
                          style={{ borderColor: `rgba(${section.rgb}, 0.08)` }}
                        />
                      </div>

                      <div className="absolute inset-0 flex items-center justify-center opacity-40 md:opacity-0 group-hover:opacity-100 group-data-[active=true]:opacity-100 transition-opacity duration-700 z-0 pointer-events-none">
                        <div
                      ...
                      {/* Icon */}
                      <div className="relative z-20 flex items-center justify-center group-hover:scale-110 group-data-[active=true]:scale-110 transition-transform duration-500 ease-out pointer-events-none">
                        <section.icon
