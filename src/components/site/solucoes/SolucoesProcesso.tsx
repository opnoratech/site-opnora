			...
			<section className="relative py-20 lg:py-28 bg-[#0c0c0f] overflow-hidden border-t border-b border-white/5">
				<div className="max-w-[85rem] mx-auto px-6 lg:px-12">
					...
					<div className="flex flex-col items-center text-center mb-20 lg:mb-24">
						...
						>
							Um processo testado e validado para entregar software de classe
							mundial.
						</ScrollReveal>
					</div>
					...
					<div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 lg:gap-6">
						{ARCHITECTURE_STEPS.map((step, idx) => (
							<ScrollReveal
								...
								className="relative flex flex-col group p-6 lg:p-8 rounded-2xl bg-[#131318]/50 border border-white/5 hover:bg-[#131318] hover:border-white/10 data-[active=true]:bg-[#131318] data-[active=true]:border-white/10 transition-all duration-500 overflow-hidden cursor-pointer md:cursor-default"
								style={
									{
							...
							>
								<div
									className="absolute inset-0 opacity-0 group-hover:opacity-100 group-data-[active=true]:opacity-100 transition-opacity duration-700 pointer-events-none"
									style={{
										background:
								...
								<div className="absolute top-0 left-0 w-full h-[2px] opacity-20 group-hover:opacity-100 group-data-[active=true]:opacity-100 transition-opacity duration-500 overflow-hidden">
									<div className="absolute inset-0 bg-gradient-to-r from-transparent via-[var(--step-color)] to-transparent -translate-x-full group-hover:animate-aurora-beam group-data-[active=true]:animate-aurora-beam" />
								</div>

								{/* Header (Number + Title) */}
								<div className="relative z-10 flex flex-col gap-5 mb-5 pointer-events-none md:pointer-events-auto">
									<div className="flex-shrink-0 size-12 rounded-full bg-[#0a0a0c] border border-white/10 flex items-center justify-center font-mono text-sm font-bold text-slate-500 group-hover:text-[var(--step-color)] group-hover:border-[var(--step-color)]/50 group-hover:shadow-[0_0_20px_rgba(var(--step-color-rgb),0.2)] group-data-[active=true]:text-[var(--step-color)] group-data-[active=true]:border-[var(--step-color)]/50 group-data-[active=true]:shadow-[0_0_20px_rgba(var(--step-color-rgb),0.2)] transition-all duration-500">
										0{idx + 1}
									</div>
									<h3 className="font-display text-xl font-bold text-white group-hover:text-[var(--step-color)] group-data-[active=true]:text-[var(--step-color)] transition-colors duration-300">
