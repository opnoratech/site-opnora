...
export function useContatoSimulador(
	defaultPlano?: string,
	defaultNivel?: string,
) {
	...
	const [basePlano, setBasePlano] = useState<string>(() => {
		if (
			defaultPlano &&
			["landing", "essencial", "profissional"].includes(defaultPlano)
		) {
			return defaultPlano;
		}
		return "sobmedida";
	...
	const handleBasePlanoChange = (planoId: string) => {
		...
		if (planoId === "sobmedida") {
			...
			if (
				...
				window.location.search.includes("plano=")
			) {
	...
	const handleSupportMonthsChange = (amount: number, e: React.MouseEvent) => {
		e.stopPropagation();
		const currentVal =
			featureLevels["Plano mensal de evolução/suporte"] || "1 mês";
		const match = currentVal.match(/\d+/);
		const currentCount = match ? parseInt(match[0], 10) : 1;
		const newCount = Math.max(1, currentCount + amount);
		setFeatureLevels((prev) => ({
	...
	const generateWhatsappUrl = () => {
		...
		const baseUrl = settings.whatsapp || "https://wa.me/5585999973965";
			...
			const { error: dbError } = await supabase.from("leads").insert([
				{
					...
					tipo: formData.investimento, // Simulador usa "investimento" no tipo por padrão ou podemos por "simulador"
					mensagem: formData.descricao,
					is_simulador: true,
					simulador_data: simuladorData,
				},
			]);

			if (dbError) {
				console.error("Erro ao salvar lead do simulador no Supabase:", dbError);
			}
		...
		selectedFeatures.forEach((feat) => {
			...
			if (!isInc) {
				if (feat === "Tráfego pago e campanhas") {
					extraMonthly +=
						currentLevel === "Gestão de tráfego + Escala" ? 700 : 350;
					...
					extraMonthly +=
						currentLevel === "Automação avançada com IA" ? 600 : 300;
	...
	const standardFeatures =
		basePlano !== "sobmedida" ? getIncludedFeatures(basePlano) : [];
	// Recursos padrão que foram desativados
	const removedFeatures = standardFeatures.filter(
