...
function PublicProposalPage() {
  ...
  const servicosList = proposal.servicos
    .split(",")
    .map((s) => s.trim())
    .filter(Boolean);
  ...
  return (
    <div className="min-h-screen bg-[#0A0A0F] text-white selection:bg-purple-500/30 font-sans pb-24">
      ...
      <div className="relative pt-12 pb-20 px-6 lg:px-8 overflow-hidden">
        <div className="absolute top-0 left-1/2 -translate-x-1/2 w-[800px] h-[300px] bg-purple-600/20 blur-[60px] md:blur-[120px] rounded-full pointer-events-none" />
        <div className="max-w-3xl mx-auto relative z-10 flex flex-col items-center text-center">
          ...
          <p className="text-xl text-slate-400 mt-4 max-w-xl">
            ...
            <strong className="text-white">{proposal.leads?.nome}</strong>{" "}
            {proposal.leads?.empresa && `da ${proposal.leads.empresa}`}.
          </p>
        </div>
      </div>
      ...
      <div className="max-w-3xl mx-auto px-6 relative z-10 space-y-8">
        ...
        <section className="bg-gradient-to-br from-purple-600/20 to-blue-600/20 border border-purple-500/30 rounded-2xl p-8 md:p-12 text-center relative overflow-hidden">
          ...
          <h2 className="text-2xl font-display font-bold mb-2 text-purple-100">
            Investimento Total
          </h2>
          <p className="text-sm text-purple-200/60 mb-6 uppercase tracking-wider font-semibold">
            Garantia Opnora Inclusa
          </p>
        ...
        <div className="text-center mt-16 pt-8 border-t border-white/10">
          ...
          <a
            href="mailto:contato@opnoratech.com"
  ...
  if (status === "aceita") {
    return (
      <span className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-emerald-500/10 text-emerald-400 border border-emerald-500/20 text-sm font-medium">
        <CheckCircle2 className="size-4" /> Proposta Aceita
      </span>
    );
  }
  if (status === "recusada") {
    return (
      <span className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-red-500/10 text-red-400 border border-red-500/20 text-sm font-medium">
        Proposta Recusada
      </span>
    );
  }
  return (
    <span className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-purple-500/10 text-purple-400 border border-purple-500/20 text-sm font-medium animate-pulse">
