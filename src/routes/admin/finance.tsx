...
import { DollarSign, Plus, ArrowUpRight, ArrowDownRight, TrendingUp, Wallet, Trash2 } from "lucide-react";
...
const CATEGORIAS_ENTRADA = ["Site Completo", "Manutenção (Mensalidade)", "Landing Page", "Identidade Visual", "Outros"];
const CATEGORIAS_SAIDA = ["Hospedagem / Servidor", "Domínio", "Ferramentas / Software", "Impostos", "Marketing / Ads", "Outros"];
		...
		<div className="space-y-6">
			...
			<div className="grid grid-cols-1 md:grid-cols-4 gap-4">
				...
				<Card className={`bg-[#131318] border-white/5 shadow-none`}>
					...
					<CardContent>
						<div className={`text-2xl font-bold ${lucroLiquido >= 0 ? "text-blue-400" : "text-red-400"}`}>
			...
			<Card className="bg-[#131318] border-white/5 shadow-none mt-6">
				...
				<CardContent className="p-0">
					<Table>
						...
						<TableBody>
							{loading ? (
								<TableRow className="border-white/5"><TableCell colSpan={5} className="text-center text-slate-500 py-8">Carregando...</TableCell></TableRow>
							) : transactions.length === 0 ? (
								<TableRow className="border-white/5"><TableCell colSpan={5} className="text-center text-slate-500 py-8">Nenhuma transação registrada.</TableCell></TableRow>
							) : (
								transactions.map((t) => (
									<TableRow key={t.id} className="border-white/5 hover:bg-white/[0.02]">
										...
										<TableCell>
											...
											{t.is_recurring && <Badge variant="outline" className="mt-1 text-[10px] bg-emerald-500/10 text-emerald-400 border-emerald-500/20">Recorrente (MRR)</Badge>}
										</TableCell>
										<TableCell>
											<Badge variant="outline" className="bg-white/5 text-slate-300 border-white/10">{t.categoria}</Badge>
										</TableCell>
										<TableCell className={`text-right font-bold ${t.tipo === "entrada" ? "text-emerald-400" : "text-red-400"}`}>
											{t.tipo === "entrada" ? "+" : "-"}{formatCurrency(t.valor)}
										</TableCell>
										<TableCell>
											<Button variant="ghost" onClick={() => deleteTransaction(t.id)} className="size-8 p-0 text-slate-500 hover:text-red-400 hover:bg-red-400/10">
				...
				<DialogContent className="bg-[#121218] border-white/10 text-white">
					...
					<form onSubmit={saveTransaction} className="space-y-4 mt-4">
						...
						<div className="flex bg-black/40 rounded-lg p-1 border border-white/5">
							<button type="button" onClick={() => setTipoForm("entrada")} className={`flex-1 py-2 text-sm font-medium rounded-md transition-colors ${tipoForm === "entrada" ? "bg-emerald-500 text-white" : "text-slate-400 hover:text-white"}`}>
								Entrada (Receita)
							</button>
							<button type="button" onClick={() => setTipoForm("saida")} className={`flex-1 py-2 text-sm font-medium rounded-md transition-colors ${tipoForm === "saida" ? "bg-red-500 text-white" : "text-slate-400 hover:text-white"}`}>
								Saída (Despesa)
							</button>
						</div>
							...
							<Select required value={categoria} onValueChange={setCategoria}>
								...
								<SelectContent className="bg-[#1a1a24] border-white/10 text-white">
									{(tipoForm === "entrada" ? CATEGORIAS_ENTRADA : CATEGORIAS_SAIDA).map(cat => (
										<SelectItem key={cat} value={cat} className="focus:bg-white/10 focus:text-white cursor-pointer">
											{cat}
										</SelectItem>
									))}
								</SelectContent>
						...
						<div className="flex justify-end gap-2 mt-6">
							...
							<Button type="submit" className={tipoForm === "entrada" ? "bg-emerald-500 hover:bg-emerald-600" : "bg-red-500 hover:bg-red-600"}>
								Salvar
							</Button>
						</div>
					</form>
