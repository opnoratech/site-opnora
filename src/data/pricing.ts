...
export const EVOLUTION_OPTIONS = [
	...
	{
		value: "completa",
		label: "Quero uma solução mais completa desde o início",
	},
...
export function isFeatureIncludedInPlan(
	...
	const getWeight = (lvl: string): number => {
		if (lvl.startsWith("Adicional") || lvl.startsWith("Sem plano mensal"))
			return 0;
		if (
		...
		if (
			...
			return 2;
		if (lvl.startsWith("Padrão") || lvl === "Fluidas" || lvl === "GA4 Padrão")
			return 3;
		if (
